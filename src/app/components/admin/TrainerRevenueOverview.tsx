...
const TrainerRevenueOverview: React.FC = () => {
  const [revenueData, setRevenueData] = useState<TrainerRevenue[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [sortConfig, setSortConfig] = useState<{ key: keyof TrainerRevenue; direction: 'ascending' | 'descending' } | null>(null);
  ...
  const fetchRevenueData = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await api.get("/admin-api/earnings-summary/?role=trainer");
  ...
  const requestSort = (key: keyof TrainerRevenue) => {
  ...
  const getSortIndicator = (key: keyof TrainerRevenue) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-2xl font-semibold text-white">Trainers Revenue Overview</h3>
        <button 
      ...
      {revenueData.length === 0 && !loading && !error && (
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 text-center">
          <p className="text-gray-300">No revenue data found for trainers.</p>
        </div>
      )}
            ...
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  >
                    <div className="flex items-center">
                      Trainer {getSortIndicator('username')}
                    </div>
                  </th>
                  <th 
                    className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600"
                  ...
                  >
                    <div className="flex items-center">
                      Orders {getSortIndicator('number_of_orders')}
                    </div>
                  </th>
              ...
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {sortedData.map((trainer) => {
                  const avgOrderValue = trainer.number_of_orders > 0 
                    ? parseFloat(trainer.total_earnings) / trainer.number_of_orders 
                    : 0;
                  
                  return (
                    <tr key={trainer.user_id} className="hover:bg-gray-700/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-gray-300">{trainer.user_id}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-green-400 font-medium">
                        {formatCurrency(trainer.total_earnings)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-gray-300">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          trainer.number_of_orders > 0 ? 'bg-blue-900 text-blue-300' : 'bg-gray-700 text-gray-300'
                        }`}>
                          {trainer.number_of_orders} orders
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-gray-300">
                        {trainer.number_of_orders > 0 ? formatCurrency(avgOrderValue.toString()) : 'N/A'}
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot className="bg-gray-700">
                <tr>
                  ...
                  <td className="px-6 py-3 text-green-400 font-medium">
                    {formatCurrency(
                      revenueData.reduce((sum, trainer) => sum + parseFloat(trainer.total_earnings), 0).toString()
                    )}
                  </td>
                  <td className="px-6 py-3 text-gray-300">
                    {revenueData.reduce((sum, trainer) => sum + trainer.number_of_orders, 0)} orders
                  </td>
                  <td className="px-6 py-3 text-gray-300">
                    {revenueData.some(t => t.number_of_orders > 0) ? 
                      formatCurrency(
                        (revenueData.reduce((sum, trainer) => sum + parseFloat(trainer.total_earnings), 0) / 
                        revenueData.reduce((sum, trainer) => sum + (trainer.number_of_orders || 0), 0)).toString()
                      ) : 'N/A'
                    }
                  </td>
                </tr>
...
export default TrainerRevenueOverview;